...
tag="${version}"
commit_tag="${RELEASE_COMMIT_SHORT_HASH:?RELEASE_COMMIT_SHORT_HASH is required}"
release_name="python-ci ${tag}"
commit_release_name="python-ci ${commit_tag}"
...

{
  ...
  echo "RELEASE_COMMIT_TAG=${commit_tag}"
  echo "RELEASE_COMMIT_NAME=${commit_release_name}"
